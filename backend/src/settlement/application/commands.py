...
@dataclass(frozen=True)
class CreatePendingEscrowCommand:
    """
    Create an escrow record in PENDING_APPROVAL state.
    Triggered by SessionAgreed domain event. No on-chain call.
    """

    session_id: uuid.UUID
    buyer_enterprise_id: uuid.UUID
    seller_enterprise_id: uuid.UUID
    agreed_price_inr: float  # INR price from negotiation


@dataclass(frozen=True)
class ApproveEscrowCommand:
    """Admin approves a pending escrow — triggers on-chain deployment."""
...
@dataclass(frozen=True)
class RejectEscrowCommand:
    """Admin rejects a pending escrow. Terminal."""
