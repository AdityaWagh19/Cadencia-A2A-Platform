...
function WalletEnabledProvider({ children }: { children: React.ReactNode }) {
   ...
   * Same signing path as the working linkWallet flow.
   */
  ...
  const signAndSubmitFundTxn = useCallback(async (escrowId: string) => {
    ...
    try {
      ...
      // 3. Build atomic group: [MBR seed, Fund payment, AppCallTxn(fund)]
      //    The contract needs MBR (0.1 ALGO) to hold state and issue inner txns.
      //    ARC-4 fund(pay)void: the payment at GroupIndex-1 is the pay arg.
      const MBR = 100_000; // 0.1 ALGO minimum balance requirement
      ...
      algosdk.assignGroupID([mbrTxn, payTxn, callTxn]);

      // 4. Sign via use-wallet
      const signedB64 = await signTxns([mbrTxn, payTxn, callTxn]);
