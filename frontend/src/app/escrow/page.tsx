...
import { Landmark, AlertCircle, History, Clock, Rocket } from 'lucide-react';
...
export default function EscrowPage() {
  ...
  const handleAction = (type: 'deploy' | 'fund' | 'release' | 'refund' | 'freeze', sessionId: string, escrowId?: string) => {
    if (type === 'fund') {
      ...
      api.post(`/v1/escrow/${escrowId}/platform-fund`)
        ...
        .catch((err: any) => {
          ...
          const msg = err?.response?.data?.detail || err?.message || 'Fund failed. Check console for details.';
          toast.error(msg);
        })
    ...
    if (type === 'deploy') {
      // Deploy via platform wallet (server-side) — no Pera Wallet needed for deploy
      setIsDeploying(true);
      toast.info('Deploying smart contract via platform wallet...');
      api.post(`/v1/escrow/${sessionId}/platform-deploy`)
        .then(() => {
          toast.success('Smart contract deployed! Proceed to fund.');
          queryClient.invalidateQueries({ queryKey: ['escrow', sessionId] });
        })
        .catch((err: any) => {
          console.error('[Deploy] failed:', err);
          const msg = err?.response?.data?.detail || err?.message || 'Deploy failed. Check console for details.';
          toast.error(msg);
        })
    ...
    if (type === 'release') {
      if (!escrowId) { toast.error('Escrow not found'); return; }
      toast.info('Releasing escrow funds via platform wallet...');
      api.post(`/v1/escrow/${escrowId}/platform-release`)
        .then(() => {
        ...
        .catch((err: any) => {
          ...
          const msg = err?.response?.data?.detail || err?.message || 'Release failed.';
          toast.error(msg);
        });
      return;
    }
        ...
        <div className="p-6 space-y-8">
          ...
          {selectedEntry && (
            <div className="bg-card border border-border rounded-lg p-6">
              ...
              {selectedEscrow ? (
                selectedEscrow.status === 'PENDING_APPROVAL' ? (
                  <div className="flex flex-col items-center justify-center py-12 text-center space-y-4">
                    <div className="bg-amber-500/10 p-4 rounded-full">
                      <Clock className="h-8 w-8 text-amber-500 animate-pulse" />
                    </div>
                    <div>
                      <h3 className="font-medium text-foreground">Pending Admin Approval</h3>
                      <p className="text-sm text-muted-foreground max-w-sm mt-1">
                        This escrow is awaiting approval from the platform administrator.
                      </p>
                    </div>
                    <div className="bg-muted rounded-lg px-4 py-2 text-xs font-mono text-muted-foreground">
                      Escrow ID: {selectedEscrow.escrow_id.slice(0, 16)}...
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col items-center justify-center py-8 text-center space-y-4">
                    ...
                    <div>
                      <h3 className="font-medium text-foreground">Admin Approved — Ready to Deploy</h3>
                      <p className="text-sm text-muted-foreground max-w-sm mt-1">
                        The escrow has been approved. Deploy the smart contract on
                        Algorand to proceed to funding.
                      </p>
                    </div>
                    <EscrowStepper
                    ...
                    />
                    {isDeploying && (
                      <p className="text-xs text-indigo-400 animate-pulse">Deploying smart contract...</p>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col items-center justify-center py-12 text-center space-y-4">
                    ...
                    <div>
                      <h3 className="font-medium text-foreground">Escrow Rejected</h3>
                      <p className="text-sm text-muted-foreground max-w-sm mt-1">
                        This escrow was rejected by the platform administrator.
                      </p>
                    </div>
                  </div>
                ) : (
                  <>
                    ...
                    />
                    {isFunding && (
                      <p className="text-xs text-indigo-400 animate-pulse mt-4 text-center">⏳ Waiting for Pera Wallet signature for fund transactions...</p>
                    )}
                  </>
                )
              ) : (
          ...
          {/* Pera Wallet Wizard — Fund step (only shown when DEPLOYED) */}
